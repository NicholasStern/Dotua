...
One of the most crucial applications of reverse auto-differentiation is backpropagation in neural networks. For some output y_hat and set of inputs X, we wish to find a function $f(x,\theta)$ that is optimized by minimizing a loss function, like MSE:
...
The architecture of a neural network, at the most basic level, is formed by an input layer, a hidden layer, and an output layer. A 'dense' neural network will involve connections between every parameter and these edges each contain a weight value. The task is to find the weights which minimize the distance between y_hat and y.

This is accomplished through a linear composition of parameters as a dense network whereby each parameter has a weight relative to every other. This captures non-linearities as certain parameters may be more expressive than others.

Backwards auto-differentation is particularly useful means of accomplishing this. This is a subspecies of reverse mode auto-differentiation. One intuitive motivation for this (h/t [Rufflewind](https://rufflewind.com/2016-12-30/reverse-mode-automatic-differentiation)) is to think of reverse mode as an inversion of the chain-rule.
...
By flipping the numerator and the denominator, this is the notation of the partial derivative of a new parameter with respect to u.
